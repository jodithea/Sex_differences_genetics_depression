...
input1_sumstats=female_MDD_sumstats.txt # 1st set of summary stats

input2_seg=maleMDD.segbfs.gz # fgwas output .segbfs.gz file from 2nd set of summary stats

input2_snp=maleMDD.bfs.gz # fgwas output .bfs file from 2nd set of summary stats

input2_sumstats=male_MDD_sumstats.txt # 2nd set of summary stats

output=female_male_MDD # name of output file that will print correlation between the two traits
...
# keeps snps in female summary stats (or male summary stats) that are also in female/male combined bfs file (keeping header)
awk '
FNR == NR {
    a[$1]
    next
