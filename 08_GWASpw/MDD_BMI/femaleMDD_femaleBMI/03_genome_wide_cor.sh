...
input1_seg=femaleMDD.segbfs.gz # fgwas output .segbfs.gz file from 1st set of summary stats 

input1_snp=femaleMDD.bfs.gz # fgwas output .bfs file from 1st set of summary stats

input1_sumstats=femaleMDD_sumstats_matched_distinct.txt # 1st set of summary stats

input2_seg=femaleBMI.segbfs.gz # fgwas output .segbfs.gz file from 2nd set of summary stats

input2_snp=femaleBMI.bfs.gz # fgwas output .bfs file from 2nd set of summary stats

input2_sumstats=femaleBMI_sumstats_matched_distinct.txt # 2nd set of summary stats

output=femaleMDD_femaleBMI # name of output file that will print correlation between the two traits
...
# keeps snps in MDD summary stats (or BMI summary stats) that are also in MDD/BMI combined bfs file (keeping header)
awk '
FNR == NR {
    a[$1]
    next
