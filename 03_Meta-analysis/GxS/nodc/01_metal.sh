...
directory=/path/03_Metal/GxS/nodc/
...
if [ $PBS_ARRAY_INDEX -eq 0 ]; then
    ...
    outfile="Metaanalysis_MDD_GxS_nodc_AllCohorts_"
    ...
    leave_out=$(basename ${files[$PBS_ARRAY_INDEX - 1]} | sed 's|_MDD_GxS_nodc_sumstats_formatted_formetaanalysis_transformed.txt||')
    config_file="01_metal_leaveout_${leave_out}.txt"
    outfile="Metaanalysis_MDD_GxS_nodc_leaveout_${leave_out}_"
