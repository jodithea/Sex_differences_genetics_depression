...
if [ $PBS_ARRAY_INDEX -eq 0 ]; then
    ...
    outfile="Metaanalysis_MDD_male_AllCohorts_"
    ...
    leave_out=$(basename ${files[$PBS_ARRAY_INDEX - 1]} | sed 's|_MDD_male_sumstats_formatted_formetaanalysis.txt||')
    config_file="01_metal_leaveout_${leave_out}.txt"
    outfile="Metaanalysis_MDD_male_leaveout_${leave_out}_"
