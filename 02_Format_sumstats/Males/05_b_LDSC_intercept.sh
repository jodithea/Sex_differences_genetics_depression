...
files=(/path/01_Format_sumstats/Males/Munged_sumstats/*_MDD_male_sumstats_formatted_formetaanalysis_LDSC_munged.sumstats.gz)

name=$(echo ${files[${PBS_ARRAY_INDEX}]} | sed 's|/path/01_Format_sumstats/Males/Munged_sumstats/||' | sed 's|_MDD_male_sumstats_formatted_formetaanalysis_LDSC_munged.sumstats.gz||')
...
ldsc.py \
 ...
 --out ${name}_MDD_male_sumstats_formatted_formetaanalysis_LDSC_h2

# eur_w_ld_chr directory provided by the software

# Create output with info on intercept

echo "LDSC Intercept and SD:" >> ${name}_MDD_male_sumstats_formatted_formetaanalysis_LDSCintercept.txt
grep "Intercept:" ${name}_MDD_male_sumstats_formatted_formetaanalysis_LDSC_h2.log >> ${name}_MDD_male_sumstats_formatted_formetaanalysis_LDSCintercept.txt

intercept=$(grep "Intercept:" ${name}_MDD_male_sumstats_formatted_formetaanalysis_LDSC_h2.log | awk '{print $2}')
sd=$(grep "Intercept:" ${name}_MDD_male_sumstats_formatted_formetaanalysis_LDSC_h2.log | awk '{print $3}' | sed 's/[()]//g')
lower_ci=$(echo "$intercept - (1.96 * $sd)" | bc)
upper_ci=$(echo "$intercept + (1.96 * $sd)" | bc)
echo "LDSC Intercept 95% CI: Lower = $lower_ci, Upper = $upper_ci" >> ${name}_MDD_male_sumstats_formatted_formetaanalysis_LDSCintercept.txt
