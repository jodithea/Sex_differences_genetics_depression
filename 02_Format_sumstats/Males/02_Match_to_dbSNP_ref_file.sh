...
files=(/path/01_Format_sumstats/Males/*_MDD_male_sumstats.txt)

name=$(echo ${files[${PBS_ARRAY_INDEX}]} | sed 's|/path/01_Format_sumstats/Males/||' | sed 's|_build3._MDD_male_sumstats.txt||')
...
if [[ "${files[${PBS_ARRAY_INDEX}]}" == *"build37"* ]]; then
    ...
    awk 'NR==FNR {
         ...
         }' ${dbSNP} ${files[${PBS_ARRAY_INDEX}]} > ${name}_MDD_male_sumstats_formatted.txt
