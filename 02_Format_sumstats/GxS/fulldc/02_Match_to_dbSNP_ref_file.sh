...
directory=/path/01_Format_sumstats/GxS/fulldc/ # This is where the plots will be output
...
files=(/path/01_Format_sumstats/GxS/fulldc/*_MDD_GxS_fulldc_sumstats.txt)

name=$(echo ${files[${PBS_ARRAY_INDEX}]} | sed 's|/path/01_Format_sumstats/GxS/fulldc/||' | sed 's|_build3._MDD_GxS_fulldc_sumstats.txt||')
...
if [[ "${files[${PBS_ARRAY_INDEX}]}" == *"build37"* ]]; then
    ...
    awk 'NR==FNR {
         ...
         }' ${dbSNP} ${files[${PBS_ARRAY_INDEX}]} > ${name}_MDD_GxS_fulldc_sumstats_formatted.txt
...
awk '($16 != "missing" && !($16 ~ /^no_match/) && $17 != "no_rsid") {print}' ${name}_MDD_GxS_fulldc_sumstats_formatted.txt > ${name}_MDD_GxS_fulldc_sumstats_formatted_formetaanalysis.txt
